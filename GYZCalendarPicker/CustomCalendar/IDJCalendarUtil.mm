...
+ (int)weekDayWithChineseYear:(NSUInteger)_year month:(NSString *)_month day:(NSUInteger)_day {
    ...
    return [IDJCalendarUtil weekDayWithSolarYear:dc.year month:[NSString stringWithFormat:@"%d", dc.month] day:dc.day];
}
...
+ (NSMutableDictionary *)jieqiWithYear:(NSUInteger)_year {
    ...
    for (int i=1; i<=24; i++) {
        ...
        [dic setObject:[arrayJieqi objectAtIndex:i-1] forKey:[NSString stringWithFormat:@"%@-%d", month, dc.day]];
    }
    return dic;
}
